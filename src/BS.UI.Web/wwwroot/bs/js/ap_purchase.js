...
$(document).on('click', '#savePurchase', function (e) {
    ...
    if (!$("#frmPurchase").valid()) {
        console.log("invalid form purchase");
        return false;
    }

    var tableSize = $('#childTableBody tr').length;
    if (tableSize == 0) {
        alert('Purchase should have atleast one Product detail');
        return false;
    }
    ...
    let billid = parseInt(document.getElementById("billid").value);
    ...
    for (let i = 1; i < table.rows.length; i++) {
        ...
        appurchaseDetails.push(appurchaseDetail);
    }

    const requestData = {
        purchase: purchaseData,
        purchaseItems: appurchaseDetails,
    };
    ...
    $.ajax({
        ...
        dataType: 'json',
        url: (window.location.origin + '/AP/Purchase/SavePurchase'),
        data: requestData,
        success: function (response) {
            $('#loader').hide();
            if (response.success) {
                ...
                if (challanId === 0) {
                    ...
                    document.getElementById("billid").value = challanId;

                    let redirectUrl = window.location.origin + "/AP/Purchase/Index";
                    setTimeout(function () {
...
function SetupFormValidation() {
    ...
    $("#frmPurchase").validate({
        ...
        messages: {
            ...
            billno: {
                required: "Enter Bill",
                maxlength: "Bill length should not exceed 20 characters"
            },
            billdate: {
            ...
            grandTotal: {
                required: "Purchase bill should have valid amount"
            }
        }
    });
