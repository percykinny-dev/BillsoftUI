...
//namespace BS.UI.Web.Areas.AP.Controllers
{
    [Area("AP")]
    [Route("AP/[controller]")]
    public class QuotationController : BSBaseController<QuotationController>
    {
        readonly ResourceManager _resourceManager;
        readonly INotyfService _notifyService;
        readonly IARChallanService challanService;

        public QuotationController(ResourceManager resourceManager,
        ...
        [HttpPost]
        [Route("GetQuotationsList")]
        public async Task<ActionResult> GetQuotationsList()
        {
        ...
        [Route("QuotationAddEdit")]
        //public ActionResult ItemAddEdit(int ItemId)
        public async Task<ActionResult> QuotationAddEdit(int? challanId)
        {
            ViewBag.Resources = _resourceManager.GetResources();
        ...
        [HttpPost]
        [Route("SaveQuotation")]
        public async Task<ActionResult> SaveQuotation(ARChallan challan, IEnumerable<ARChallanDetail> challanItems)
        {
            try
            {
                ...
                //var result = await challanService.SaveQuotation(challan, challanItems);

                await Task.CompletedTask;
                //if (!result.IsSuccess)
                //    throw new BSInfrastructureException(result.Messages[0].IsNull() ? "error saving challan information" : result.Messages[0]);

                _notifyService.Success("Quotation data saved successfully");
                return Json(new { success = true, data = new { QuotationId = 101 } });
                //return Json(new { success = true, data = new { QuotationId = challan.QuotationID } });
            }
            catch (Exception ex)
            {
                var msg = "an error occurred while saving Quotation data";
                LogException(msg, ex);
                _notifyService.Warning(msg);
                return Json(new { success = false, data = new { QuotationId = challan.ChallanID } });
            }
        }
